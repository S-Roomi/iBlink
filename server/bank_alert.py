...
class BlinkListenerST:
    ...
    def __init__(self, cfg: dict):
        ...
        self.amp_history    = deque(maxlen=maxlen)   # kept for the chart only
        self.phase_history  = deque(maxlen=maxlen)   # kept for the chart only
        self.deriv_history  = deque(maxlen=maxlen)   # blink feature (phase deriv)
    ...
    def _process_chirp(self, rx_chunk) -> Optional[complex]:
        ...
        if self._static_clutter is None:
            ...
            if len(self._calib_spectra) >= self._calibration_n:
                ...
                # Lock face bin: highest energy in distance window after clutter sub
    ...
    def _detect_blink(self, face_phasor: complex) -> bool:
        ...
        Paper-accurate blink detection:
          1. Extract phase; unwrap continuously.
          2. High-pass filter to remove breathing drift.
          3. Compute first-order phase derivative (= instantaneous phase velocity).
          4. Smooth with a short window.
          5. Adaptive threshold (EMA on non-blink frames).
        ...
        # Need at least a few samples for the derivative
        if len(self._phase_raw) < 4:
            return False

        # ── Step 1: breathing removal on the phase history buffer ─────────────
        phase_arr = np.array(self._phase_raw)
        clean_ph  = self._remove_breathing(phase_arr)

        # ── Step 2: absolute first-order derivative of clean phase ────────────
        # Use the last 2 samples for the instantaneous derivative
        d_phase = abs(clean_ph[-1] - clean_ph[-2])
        self.deriv_history.append(d_phase)

        # ── Step 3: smooth the feature ────────────────────────────────────────
        ...
        # Update background only on quiet frames (feature below threshold).
        # This correctly tracks slow baseline drift without being fooled by blinks.
        if self._bg_level is None:
            self._bg_level = feature
        else:
            threshold_now = self._bg_level * self.threshold_mult
            if feature < threshold_now:
                # Quiet frame — update background
                self._bg_level = (1.0 - self._bg_alpha) * self._bg_level + self._bg_alpha * feature
    ...
    def build_figure(self):
        ...
        # Compute the breathing-cancelled phase for display
        ...
        axes[1].plot(t, phases,       lw=0.7, color="#888", alpha=0.5, label="Raw phase (unwrapped)")
        axes[1].plot(t, clean_phases, lw=0.9, color="#ffb74d",         label="Phase (breathing removed)")
        ...
        axes[2].plot(t, thresh_arr, lw=1.2, linestyle="--", color="#ef5350", label="Adaptive threshold")
        if len(peaks):
...
**3. Mix (FMCW)** — TX × RX produces a *beat* frequency proportional to reflector distance.
    ...
    with col_b:
        st.markdown("""
**4. Range FFT** — Finds your face bin within the distance window.

**5. Phase & Amplitude tracking** — An eye-blink causes a sub-mm eyelid displacement detected as a transient change.
