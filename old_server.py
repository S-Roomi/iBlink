    ...
    for i in range(pa.get_device_count()):
        try:
            ...
            devices.append(DeviceInfo(
                ...
                default_sample_rate = float(info.get("defaultSampleRate", 0.0)) or None,
...
class BlinkListener:
    ...
    def __init__(self, cfg: SessionConfig):
        ...
        self._bg_alpha = 0.02
        self._last_feature: Optional[float] = None
    ...
    def _detect_blink(self, face_phasor: complex) -> bool:
        ...
        self._last_threshold = detection_gate
    ...
    def start(self, device_index: Optional[int] = None):
        ...
        try:
            if self.selected_device_index is not None:
                info = self.audio.get_device_info_by_index(self.selected_device_index)
            else:
                info = self.audio.get_default_input_device_info()
                self.selected_device_index = int(info.get("index")) if info.get("index") is not None else None
            self.selected_device_name = info.get("name")
        except Exception:
    ...
    def snapshot(self) -> StatusResponse:
        ...
        calibration_remaining_s = 0.0
        if self._start_wall_time is not None and not self._sensing_enabled:
            calibration_remaining_s = max(0.0, self.sensing_delay_s - (time.time() - self._start_wall_time))
        ...
        return StatusResponse(
            ...
            calibration_remaining_s = calibration_remaining_s,
            selected_device_index = self.selected_device_index,
            selected_device_name = self.selected_device_name,
            feature_value      = self._last_feature,
            threshold_value    = self._last_threshold,
