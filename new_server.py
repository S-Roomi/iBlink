...
class SessionConfig(BaseModel):
    ...
    blink_feature_floor: float = Field(DEFAULTS["blink_feature_floor"], ge=0.1, le=10.0)
...
class BlinkListener:
    ...
    def __init__(self, cfg: SessionConfig):
        ...
        self.blink_feature_floor = cfg.blink_feature_floor
        ...
        self.selected_device_index: Optional[int] = cfg.device_index
        self.selected_device_name: Optional[str] = None

        self.feature_value: Optional[float] = None
        self.threshold_value: Optional[float] = None

    def _reset_runtime_state(self):
        self.face_bin = None
        self._calib_spectra.clear()
        ...
        self._peak_feature = 0.0
        self.blink_count = 0
        self._chirps_since_blink = int(self.refractory_s / self.chirp_dur)
        self._chirp_index = 0
    ...
    def _detect_blink(self, face_phasor: complex) -> bool:
        ...
        detection_gate = max(threshold, self.blink_feature_floor)
        self.threshold_value = detection_gate
    ...
    def start(self, device_index: Optional[int] = None):
        ...
        try:
            if device_index is not None:
                ...
                self.selected_device_name = str(info.get("name", f"Device {device_index}"))
        except Exception:
        ...
        if self.selected_device_name is None:
            try:
                info = self.audio.get_default_input_device_info()
                self.selected_device_index = int(info.get("index", -1))
                self.selected_device_name = str(info.get("name", "System default"))
            except Exception:
                self.selected_device_name = "System default"
    ...
    def snapshot(self) -> StatusResponse:
        ...
        if self._running and self._static_clutter is None:
            remaining_chirps = max(0, self._calibration_n - len(self._calib_spectra))
            calibration_remaining = remaining_chirps * self.chirp_dur
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("new_server:app", host="0.0.0.0", port=8000, reload=True)
